...
(function($){
	...
	var t_methods = {
        ...
        if(window.console&&settings.my_debug){			
			...
			var my_styel_4_resize_image=function(my_scale){
			...
			//var 
			var my_500px_window_function=function(){
			...
			var my_scale_items_12=function(my_scale){
     	//	if(my_w_12<600){
	...
	//		}
			...
			var my_autoplay_function_12=function(){
				//var $this=this;
			...
			$this.find(".t_left , .t_right").on("mouseenter",function(e){
				//return;
				var $p=$(this);
				var my_slider_set_timeout_12=setTimeout(function(e){
				//var $p=$(e.target);	
				$p.finish();
			...
			var my_slider_mouse_leave_arrows=function(e){
				//return;
			...
			var my_center_style_3=function(){
			...
			// Bind left on click
			$this.find('.t_left').on('click', function(){
				var data = $this.data('timeline'),
			...
			// Bind right on click
			$this.find('.t_right').on('click', function(){
				//$this.find(".timeline_items").finish();
				var data = $this.data('timeline'),
				speed = data.options.scrollSpeed;
				//data.margin-=100;
			...
			// Bind close on click
			$this.find('.t_close').on('click', function(){
		...
		touchEnd : function(xpos) {
			...
			if(typeof data.touchHorizontal != 'undefined' && data.touchHorizontal) {
				...
				itemC -= parseInt(xmargin/itemWidth,10);
		...
		/*	if(my_width<610){
			...
			if(my_curr>0){
				...
				//$items.each(function(){
				//	var my_count=parseInt($(this).attr('data-count'),10);
				//	var ci12=0;
...
//						
		...
		goTo : function (id, data_count, openElement) {
			...
			if(found)
			{
				...
				var multiply = (parseInt(data.iholder.css('margin-left'),10) - data.margin)/data.itemWidth;				
				/*if(data.my_resize_12!='undefined' && data.my_resize_12){
...
'	 </div>\n';
			...
			var	timelineWidth = $this.find('.timeline_line').width(),
				...
				var monthsDays = [0].concat(data.options.nuberOfSegments),
				minM = months.length,
				minY = 99999,
				maxM = 0,
				maxY = 0;
...
'		</div>\n';
			...
			$this.find('#t_line_left').on('click', function(){
				$this.timeline('lineLeft');
			});
			
			$this.find('#t_line_right').on('click', function(){
